...
public class LargeOakFoliagePlacer extends FoliagePlacer{

    public static final Codec<LargeOakFoliagePlacer> CODEC = RecordCodecBuilder.create(a -> makeCodec(a).apply(a, LargeOakFoliagePlacer::new));

    protected static <P extends LargeOakFoliagePlacer> Products.P3<RecordCodecBuilder.Mu<P>, FeatureSpread, FeatureSpread, Integer> makeCodec(RecordCodecBuilder.Instance<P> builder) {
        return func_242830_b(builder).and(Codec.intRange(0, 24).fieldOf("height").forGetter((placer) -> placer.height));
    }

    protected final int height;

    public LargeOakFoliagePlacer(FeatureSpread radius, FeatureSpread offset, int height){
        super(radius, offset);
        this.height = height;
    }
