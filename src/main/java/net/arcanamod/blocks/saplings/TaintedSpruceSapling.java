...
import net.arcanamod.worldgen.trees.TaintedMegaSpruceGenerator;
		...
		for(x = 0; x >= -1; --x){
			for(z = 0; z >= -1; --z){
				if(this.isTwoByTwoOfType(worldIn, pos, x, z, BlockPlanks.EnumType.SPRUCE)){
					worldgenerator = untainted ? new TaintedMegaSpruceGenerator(false, rand.nextBoolean(), true) : new TaintedMegaSpruceGenerator(false, rand.nextBoolean(), false);
					flag = true;
					break;
				}
			}
