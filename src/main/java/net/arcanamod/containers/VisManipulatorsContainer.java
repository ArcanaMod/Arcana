...
public class VisManipulatorsContainer{// extends AspectContainer{
	...
	protected void refreshAspectSlots(){
		...
		Supplier<VisHandler> left = () -> VisHandler.getFrom(getSlot(0).getStack());
		if(left.get() != null){
		...
		Supplier<VisHandler> right = () -> VisHandler.getFrom(getSlot(1).getStack());
		if(right.get() != null){
	...
	*//**
	...
	public List<VisHandler> getOpenHandlers(){
		...
		List<VisHandler> ret = new ArrayList<>();
		VisHandler left = VisHandler.getFrom(getSlot(0).getStack());
		if(left != null)
			ret.add(left);
		VisHandler right = VisHandler.getFrom(getSlot(1).getStack());
		if(right != null)
			ret.add(right);
		return ret;
	}*/
