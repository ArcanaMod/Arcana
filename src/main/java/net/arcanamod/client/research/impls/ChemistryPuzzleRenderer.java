...
public class ChemistryPuzzleRenderer extends AbstractGui implements PuzzleRenderer<Chemistry>{
	...
	public void render(Chemistry puzzle, List<AspectSlot> puzzleSlots, List<Slot> puzzleItemSlots, int screenWidth, int screenHeight, int mouseX, int mouseY, PlayerEntity player){
		...
		for(int y = 0; y < gridHeight; y++){
			for(int x = 0; x < gridWidth; x++){
				...
				if(slot != null){
					ClientUiUtil.renderAspect(slot, scX + 2, scY + 2);
					mc().getTextureManager().bindTexture(TEX);
				}else{
					mc().getTextureManager().bindTexture(TEX);
	...
	public void renderAfter(Chemistry puzzle, List<AspectSlot> puzzleSlots, List<Slot> puzzleItemSlots, int screenWidth, int screenHeight, int mouseX, int mouseY, PlayerEntity player){
		...
		for(Integer index : puzzle.getAspectSlotIndexes()){
			...
			if(mouseX >= scX && mouseX < scX + 16 && mouseY >= scY && mouseY < scY + 16){
				ClientUiUtil.drawAspectTooltip(puzzle.getAspectInSlot(index), mouseX, mouseY, screenWidth, screenHeight);
				break;
			}
		}
	}
