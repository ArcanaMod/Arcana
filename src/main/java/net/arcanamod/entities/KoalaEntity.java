...
public class KoalaEntity extends AnimalEntity {
    ...
    protected void updateAITasks() {
        this.koalaTimer = this.eatGrassGoal.getEatingGrassTimer();
        super.updateAITasks();
    }
    ...
    public void livingTick() {
        if (this.world.isRemote) {
            this.koalaTimer = Math.max(0, this.koalaTimer - 1);
        }
        super.livingTick();
    }
    ...
    public float getHeadRotationAngleX(float p_70890_1_) {
        if (this.koalaTimer > 4 && this.koalaTimer <= 36) {
            float f = ((float) (this.koalaTimer - 4) - p_70890_1_) / 32.0F;
            return ((float) Math.PI / 5F) + 0.21991149F * MathHelper.sin(f * 28.7F);
        } else {
            return this.koalaTimer > 0 ? ((float) Math.PI / 5F) : this.rotationPitch * ((float) Math.PI / 180F);
        }
    }
}
