    ...
    public void generateTree(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        for (x = 0; x >= -1; --x)
        {
            for (z = 0; z >= -1; --z)
            {
                ...
                {
                    worldgenerator = untainted ? new TaintedMegaSpruceGenerator(false, rand.nextBoolean(), true, true) : new TaintedMegaSpruceGenerator(false, rand.nextBoolean(), true, false);
                    flag = true;
                    break;
                }
            }
